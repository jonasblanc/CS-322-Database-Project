...
--QUERY3--
SELECT COUNT(*)/(SELECT COUNT(*) from COLLISIONS) as FRACTION_DARK from COLLISIONS C
WHERE C.LIGHTING_ID IN
(SELECT L.ID from LIGHTING L
where LOWER(L.DEFINITION) LIKE '%dark%');

--QUERY4--
SELECT COUNT(*) as NUMBER_COLLISIONS_SNOWY_WEATHER from COLLISION_WITH_WEATHER CWW
...
SELECT W.DEFINITION, COUNT(*) as NUMBER_COLLISIONS from WEATHER W, COLLISION_WITH_WEATHER CWW
...
(Select CWRC.CASE_ID from COLLISION_WITH_ROAD_CONDITION CWRC
...
WHERE V2.ID in
(SELECT VEWSE.VICTIM_ID from VICTIM_EQUIPPED_WITH_SAFETY_EQUIPMENT VEWSE
