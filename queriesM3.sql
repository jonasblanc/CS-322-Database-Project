...
-- Query 2
SELECT SWT.DEFINITION, STATS_COLLISIONS_HOLE.NUMBER_OF_COLLISION_HOLE
...
SELECT CASE WHEN ((EXTRACT(MONTH FROM C.COLLISION_DATE) BETWEEN '4' AND '8'
        ...
        WHEN (EXTRACT(MONTH FROM C.COLLISION_DATE) BETWEEN '4' AND '8'
         ...
         OR (C.COLLISION_TIME IS NULL
          AND C.LIGHTING_ID = L.id AND L.DEFINITION = 'Daylight') THEN 'DAY_COLLISIONS'
        ELSE 'NIGHT_COLLISION'
    END, COUNT(*) AS NUMBER_ACCIDENT
FROM COLLISIONS C, LIGHTING L
WHERE (C.COLLISION_DATE IS NOT NULL
      AND C.COLLISION_TIME IS NOT NULL)
      OR (C.COLLISION_TIME IS NULL AND C.LIGHTING_ID = L.id AND L.DEFINITION = 'Daylight')
GROUP BY (
