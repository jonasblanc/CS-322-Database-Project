...
SELECT distinct COUNTY_CITY_LOCATION,
                POPULATION_ID,
                avg(v.VICTIM_AGE) OVER (PARTITION BY COUNTY_CITY_LOCATION, C.CASE_ID) as v_age
FROM COLLISIONS C
         INNER JOIN PARTIES P on C.CASE_ID = P.COLLISION_CASE_ID
         inner join VICTIMS V on P.ID = V.PARTY_ID
WHERE C.COUNTY_CITY_LOCATION in (
    SELECT distinct COUNTY_CITY_LOCATION
    from COLLISIONS C
    INNER JOIN POPULATION P ON P.ID = C.POPULATION_ID
...
select Row_Number, COUNTY_CITY_LOCATION,P.DEFINITION, ROUND(V_AGE,2)
from rws
