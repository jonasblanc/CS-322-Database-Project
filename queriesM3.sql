...
FROM (
         ...
         FROM VICTIM_SEATING_POSITION VSP,
              (
                  SELECT V.VICTIM_SEATING_POSITION_ID, COUNT(*) AS NUMBER_NO_INJURIES
                  FROM VICTIMS V
                  WHERE V.VICTIM_DEGREE_OF_INJURY_ID IN (
                      SELECT VDOI.ID
                      FROM VICTIM_DEGREE_OF_INJURY VDOI
     ...
     (
         ...
         FROM VICTIM_SEATING_POSITION VSP,
              (
                  ...
                  FROM VICTIMS V
                  WHERE VICTIM_SEATING_POSITION_ID is not NULL
...
SELECT C.CASE_ID, MAX(V.VICTIM_AGE)
    ...
    WHERE  V.PARTY_ID = P.ID
      ...
      AND C.TYPE_OF_COLLISION_ID = 'G' --TOC.ID
      --AND LOWER(TOC.DEFINITION) = 'vehicle/pedestrian'
GROUP BY CASE_ID
