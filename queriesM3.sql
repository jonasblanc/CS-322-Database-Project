...
(
    ...
    FROM PARTIES P
    WHERE P.STATEWIDE_VEHICLE_TYPE_ID IS NOT NULL AND P.COLLISION_CASE_ID IN 
        (   SELECT C.CASE_ID 
            FROM COLLISIONS C
            WHERE C.CASE_ID IN 
                (   SELECT CWRC.CASE_ID
...
CREATE VIEW SEATING_POSITION_TO_SAFETY_FACTOR AS
    SELECT
        UNINJURED.DEFINITION, ROUND(UNINJURED.NUMBER_NO_INJURIES/ALL_DEGREES.NUMBER_ALL_DEGREE_INJURIES, 3) AS SAFTEY_FACTOR
        FROM (
            ...
            (
            SELECT VSP.DEFINITION as DEFINITION, GROUPED_SEATING_POSITIONS.NUMBER_ALL_DEGREE_INJURIES as NUMBER_ALL_DEGREE_INJURIES
            FROM VICTIM_SEATING_POSITION VSP, (
...
FROM    (   SELECT COUNT(*)
            FROM    (   SELECT P.STATEWIDE_VEHICLE_TYPE_ID AS TYPE, C.COUNTY_CITY_LOCATION, COUNT(*)
                        FROM PARTIES P, COLLISIONS C
                        WHERE P.COLLISION_CASE_ID = C.CASE_ID AND C.COUNTY_CITY_LOCATION IS NOT NULL
...
FROM PARTIES P, STATEWIDE_VEHICLE_TYPE SWT
WHERE P.STATEWIDE_VEHICLE_TYPE_ID IS NOT NULL AND SWT.ID = P.STATEWIDE_VEHICLE_TYPE_ID AND P.COLLISION_CASE_ID IN 
    (   SELECT C.CASE_ID 
        FROM COLLISIONS C
        WHERE C.CASE_ID IN 
            (   SELECT CWRC.CASE_ID
